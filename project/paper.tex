...
% \begin{proof}
% Proof by exhaustion with all rules involving $\prim$ types not stepping. To values
% \end{proof}
% Part 2: $\tau^* \tstep \tau^{*\prime}$ will eventually step to a $\tau^{*\prime}$ such that $\tau_i \prim ~ \forall~ \tau_i \in \tau^{*\prime}$.
% \begin{proof}
% This is provable by the fact that the language does not feature recursive types and the only form of recursion in the language is via the fix operator and $\kwType{}$ operator does no have to follow recursive paths in order to get it's type (Type Rules \Rulename{Macro}, \Rulename{Fix}). The other rules are trivial (ie - step to a $\prim$) and/or irrelevant to the assertion
% \end{proof}
